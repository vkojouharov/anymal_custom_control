...
def _build_init_points(task, points_per_target, init_mode, offset_step=0.1, max_offset_steps=50):
    ...
    previous_point = np.asarray(task["start"], dtype=float)
...
def _build_scp_problem(task, points_per_target, trust_region_radius, sample_fractions):
    ...
    for i, target in enumerate(task["targets"]):
        ...
        constraints.append(
            cp.norm(x[target_idx] - np.asarray(target["center"], dtype=float), 2)
            <= float(target["radius"])
        )
...
def solve_trajectory(TASK, pruning_threshold=0.1, intermediate_points=3, init_mode="offset_interp"):
    construct_start = time.perf_counter()

    # Retrieve task parameters
    num_tasks = len(TASK["targets"])
    num_obstacles = len(TASK["obstacles"])
    min_obstacle_size = min(obs["radius"] for obs in TASK["obstacles"]) if num_obstacles > 0 else 1.0
    ...
    for i in range(num_tasks):
        constraints.append(
            cp.norm(x[(i + 1) * points_per_target - 1] - TASK["targets"][i]["center"], 2)
            <= TASK["targets"][i]["radius"]
        )
    # ensure segments between waypoints don't penetrate obstacles
    ...
    for i in range(num_tasks * points_per_target):
        for j in range(num_obstacles):
            constraints.append(
                TASK["obstacles"][j]["radius"] - cp.norm(x[i] - TASK["obstacles"][j]["center"], 2) <= 0
            )

    if init_mode not in {"interp_line", "offset_interp"}:
    ...
    solve_result = problem.solve(method="dccp", solver=cp.SCS)
