...
def make_task(r):
    ...
    return {
        ...
        "co": np.array([0.2, 2.0]),  "ro": 0.25,  # optional obstacle
    }
...
task = {
    "x0": np.array([0.0, 0.0]),
    "c1": np.array([1.0, 1.0]),  "r1": 0.75,
    "c2": np.array([-0.5, 2.5]), "r2": 0.5,
    "c3": np.array([0.0, 4.0]),  "r3": 1.0,
    "co": np.array([1.0, 3.0]),  "ro": 1.5,  # optional obstacle
}
...
def plot_task(task, optimal_traj=None, save_path=None):
    ...
    # Obstacle (co/ro), if present
    if "co" in task and "ro" in task:
        obs_center = np.asarray(task["co"])
        obs_radius = float(task["ro"])
        ax.add_patch(
            Circle(
                obs_center,
                obs_radius,
        ...
        all_x.append(obs_center[0])
        all_y.append(obs_center[1])
        all_r.append(obs_radius)
