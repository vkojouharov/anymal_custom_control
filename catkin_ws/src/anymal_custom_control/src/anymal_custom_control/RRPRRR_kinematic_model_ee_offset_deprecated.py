...
## -------------------- END-EFFECTOR OFFSET --------------------
# Fixed translation from frame 6 origin to the actual end-effector, expressed
# in frame 6 coordinates. Edit these for the new tool/gripper geometry.
EE_OFFSET_X_CONST = 0.05  # m
EE_OFFSET_Y_CONST = 0.0  # m
EE_OFFSET_Z_CONST = 0.0  # m
...
MDH_sym = {
    ...
    4: {'a': 0,         'al': 0,       'd': 0,        'th': th4},
    5: {'a': L4_CONST, 'al': sp.pi/2, 'd': 0,        'th': th5},
    6: {'a': 0,         'al': sp.pi/2, 'd': 0,        'th': th6}    
}
...
def sym_ee_offset_transform():
    return sp.Matrix([
        [1, 0, 0, EE_OFFSET_X_CONST],
        [0, 1, 0, EE_OFFSET_Y_CONST],
        [0, 0, 1, EE_OFFSET_Z_CONST],
        [0, 0, 0, 1],
    ])
...
def sym_forward_kinematics_corrected(MDH):
    ...
    T6E = sym_ee_offset_transform()
    T = T01 @ T12 @ T23 @ T3d @ T34 @ T45 @ T56 @ T6E
    return T
